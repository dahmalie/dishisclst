...
container: "/users/singularity/dishisclust.latest.sif"

# mount project folder
workflow.singularity_args  = "-B /users/projects/pmh-cluster "
...
rule make_sparse_network:
     input: rules.make_dense_network.output["mci"]
     output: "interim/03-psn/{co}-{cnn}.mci"
     benchmark: "benchmarks/03-psn-{co}-{cnn}-create.log"
     resources: vmem=1024*30, tmin=180
     shell: """
...
# Run mcl clustering
rule run_clustering:
     input: rules.make_sparse_network.output
     output: "interim/04-cls/{co}-{cnn}-{pi}-{ip}.icl"
     resources: vmem=1024*50, tmin=280
     threads: 4
     shell: """
         mcl {input} -o {output} \
...
rule dump_network_edges:
     ...
     output: "interim/03-psn/{co}-{cnn}.abc"
     resources: vmem=1024*50, tmin=10
     shell: """
...
rule get_cluster_memberships:
	input:
		...
		tab = rules.make_dense_network.output["tab"]
	output: "interim/04-cls/{co}-{cnn}-{pi}-{ip}.membs"
	resources: vmem=1024*15, tmin=10
	shell: """
...
rule aggregate_clusterings:
     input:
         clst = [ 
             f"interim/04-cls/{{co}}-{{cnn}}-{{pi}}-{ip}.membs"
                 for ip in ip_list 
         ]
     output: 
         all = "output/mcl_{co}-{cnn}-{pi}.tsv"
     resources: vmem=1024*5, tmin=10
     script: "scripts/combine_clusterings.R"

# Create sankey plots
rule sankey_plot:
     input: "output/mcl_{co}-{cnn}-{pi}.tsv"
     output: "sankey_{co}-{cnn}-{pi}.pdf",
     resources: vmem=512*2, tmin=15
     script: "scripts/create_sankeyplot.R"
